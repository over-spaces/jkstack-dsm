...
public class ServiceCategoryController extends BaseController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(paramType = "body", dataType = "ServiceCategoryVO", name = "serviceCategoryVO"),
    })
    @ApiOperation(value = "新建/修改服务类别列表")
    @PostMapping("/save")
    public ResponseResult save(@RequestBody @Valid ServiceCategoryVO serviceCategoryVO) {
        if (StringUtils.isBlank(serviceCategoryVO.getServiceCategoryId())) {
            serviceCategoryService.save(serviceCategoryVO.toServiceCategoryEntity());
        } else {
            ServiceCategoryEntity serviceCategoryEntity = serviceCategoryService.getByBusinessId(serviceCategoryVO.getServiceCategoryId());
            serviceCategoryEntity.setName(serviceCategoryVO.getName());
            serviceCategoryEntity.setParentServiceCategoryId(serviceCategoryVO.getParentServiceCategoryId());
            serviceCategoryEntity.setDescription(serviceCategoryVO.getDescription());
            serviceCategoryService.updateById(serviceCategoryEntity);
        }
